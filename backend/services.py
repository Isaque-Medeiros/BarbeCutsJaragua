...
def brasilia_now() -> datetime:
    ...
    Retorna um datetime sem informação de timezone (naive) para compatibilidade
    com comparações contra datetimes vindos de ISO strings (que também são naive).
    """
...
def validar_agendamento(
    ...
    # Validar bloqueios
    for bl in bloqueios:
        # Extrair apenas a data da string data_hora_inicio (YYYY-MM-DDTHH:MM:SS ou YYYY-MM-DD HH:MM:SS)
        data_agendamento = data_hora_inicio.split('T')[0].split(' ')[0]
